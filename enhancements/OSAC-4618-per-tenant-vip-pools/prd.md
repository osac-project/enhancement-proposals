...
- For Dev Preview, onboarding operations are serialized per instance — processed one at a time within a single OSAC instance. Because Dev Preview supports exactly one OSAC instance per storage backend (see Assumptions), per-instance serialization is sufficient to prevent two operations from claiming the same pool. The storage backend's lack of server-side concurrency protection is an accepted platform constraint; duplicate pool bindings are never acceptable at any milestone. GA must support multiple concurrent OSAC instances per storage backend, requiring cross-instance coordination beyond per-instance serialization; the mechanism is deferred to the design EP.
...
- Dev Preview supports a single OSAC instance per storage backend. Per-instance serialization is sufficient at this concurrency level.
- The storage backend does not prevent concurrent pool assignments; OSAC must ensure each pool is assigned to exactly one tenant. For Dev Preview, this is mitigated by processing onboarding operations one at a time within the single instance (see above). Production-grade concurrent-safe binding must be implemented before GA; the mechanism is deferred to the design EP.
