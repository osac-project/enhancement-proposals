...
Capabilities declare what a NetworkClass can provide. They are used for
validation (reject unsupported configurations), discovery (UI/CLI), and
documentation.
...
The NetworkClass capabilities must be consistent with the capabilities of
its assigned managers. The operator validates this at NetworkClass creation
time.
...
metadata:
  ...
  namespace: osac
  labels:
    osac.openshift.io/fabric-manager: "true"
data:
  name: netris
...
available interfaces by inspecting the template:

```bash
osac get baremetalinstancetemplate bcm_h100 -o yaml
# spec_defaults.interfaces:
# - name: data-0
#   description: "25Gbps data interface"
# - name: data-1
#   description: "25Gbps data interface"
# - name: mgmt
#   description: "1Gbps management interface"
```

The tenant then specifies which interface to attach to which subnet. Each
...
fabric segment. Each interface gets an IP from its subnet's CIDR. All
referenced subnets must belong to the same VirtualNetwork. The same
interface cannot appear in multiple attachments.
...
The fabric manager creates a DNAT rule: external IP → resource's fabric IP.
The resource is directly on the fabric — no intermediary bridge needed,
regardless of type.
...
message BareMetalNetworkAttachment {
  string subnet = 1;                    // Subnet ID, required, immutable
  repeated string security_groups = 2;  // SecurityGroup IDs, optional, mutable
  string interface = 3;                 // optional, immutable: physical interface name from template
}
```

Each entry maps one physical interface to one subnet. The `interface`
field references a named interface from the `BaremetalInstanceTemplate`.
If omitted, the fabric manager picks a default. See the
[BaremetalInstance E2E flow](#baremetalinstance) for interface discovery
and multi-interface examples.
