...
- **CatalogItems integration**: CatalogItems (see `/enhancements/OSAC-1002-catalog-items`) may reference ComputeImages as building blocks — e.g., a catalog entry could combine a ComputeImage with a predefined instance size and other supported non-network fields into a one-click deployment template. Resource networking remains part of the ordinary resource provisioning flow.
