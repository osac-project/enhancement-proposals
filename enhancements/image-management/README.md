...
1. **Transitional release**: The old fields (`source_type`/`source_ref`) and maintained for existing resources. The new `compute_image` field is required on ComputeInstance creation and update. The old fields are marked `deprecated` in the proto definition and API documentation, with a clear message directing users to `compute_image`.
...
- **Deleting a ComputeImage**: OSAC uses soft-delete — the record is marked with `deleted_at` and excluded from queries, but retained in the database. Existing VMs using the soft-deleted image continue to run. Only new VM creation referencing it will fail. This is expected behavior. To prevent deletion of in-use images, check for active references before deleting (advisory, not enforced in this milestone).
