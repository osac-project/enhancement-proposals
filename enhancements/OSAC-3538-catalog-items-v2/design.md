...
2. **No per-field type customization.** Every field's default is a `google.protobuf.Value` and its validation is a JSON Schema string. All governed fields share the same untyped value container, so `instance_type` (which should carry an `InstanceTypeReference`) and `ssh_public_key` (a plain string) are represented identically. The current model cannot enforce that a locked instance_type value is a valid reference at the proto level.
