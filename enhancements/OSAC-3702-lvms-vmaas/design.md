...
This enhancement adds LVMS (node-local LVM, via topolvm) as a storage backend behind OSAC's opaque `local` storage tier, for single-node VMaaS. It extends the existing storage control plane (OSAC-2872) additively: the OSAC CSI driver delegates `CreateVolume` to the fulfillment Volume API as it does for network backends, and a new `LvmsVendorProvisioner` in osac-operator provisions the volume by writing a topolvm `LogicalVolume` CR — which the already-installed `topolvm-node` reconciler carves into an LV on the scheduled node. The only cross-cutting API change is one optional topology message on the private Volume API. See [PRD](prd.md) for detailed requirements.
...
- Present the local tier through the uniform `osac.csi.openshift.io` provisioner with a real Volume inventory record, not a native `topolvm.io` StorageClass [Locked: D11].
...
Authored: draft @ design 0.9.0 - 562b610, workspace feat/OSAC-4046 @ d6129293e
Phases: draft, draft

<!-- ai-workflow-provenance:{"schema_version":1,"provenance_kind":"session","workflow":"design","workflow_version":"0.9.0","ai_workflows":"562b610","source_repo":"d6129293e","source_repo_branch":"feat/OSAC-4046","commits_behind_main":0,"commits_ahead_main":1,"main_ref":"main","phases":["draft","draft"],"authoring_modes":["skill"],"context_changed":false,"origin_untracked":false} -->
