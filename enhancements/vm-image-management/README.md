...
5. Fulfillment-service:
   - Stores uploaded qcow2 temporarily
   - Pushes qcow2 as OCI artifact to CSP registry (e.g., `registry.csp.internal/vm-images/org-acme/windows-2022:v1`)
   - Creates VirtualMachineImage CR with registry reference
6. Operator (on each cluster) sees new VirtualMachineImage:
   - Creates CDI DataVolume with `source.registry` pointing to the OCI artifact
   - CDI imports from registry → local PVC cache (one-time operation, ~1-2 minutes)
   - Updates VirtualMachineImage status to `Available`
...
service Images {
  ...
  // Initiate upload (creates VirtualMachineImage and returns upload token/URL)
  ...
  // Delete image (deletes backing PVC)
...
// VirtualMachineImage has no status - it's purely metadata
// Cache PVCs are created on-demand when VMs reference the image
```
...
**Risk: Registry unavailable during image import.**

Mitigation:
- Retry logic in operator for failed imports
- VirtualMachineImage status clearly indicates import failure with registry error message
- Existing VMs continue to work (cloning from already-cached PVCs)
- Manual retry: delete and recreate VirtualMachineImage CR to trigger re-import
